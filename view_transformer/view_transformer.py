...
class ViewTransformer():
    def __init__(self):
        ...
        self.target_vertecies = np.array([
            [0, court_width],
            [0, 0],
            [court_length, 0],
        ...
        self.target_vertecies = self.target_vertecies.astype(np.float32)

        self.perspective_transformer = cv2.getPerspectiveTransform(self.pixel_verticies, self.target_vertecies)
    ...
    def add_transformed_position_to_tracks(self, tracks):
        for object, object_tracks in tracks.items():
            for frame_num, track in object_tracks.items():
